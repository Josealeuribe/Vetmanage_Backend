...
export class OrdenesVentaService {
  ...
  private normalizarEstado(nombre?: string | null) {
  ...
  private esEstadoPendiente(nombre?: string | null) {
    return this.normalizarEstado(nombre) === 'pendiente';
  }

  private esEstadoAprobada(nombre?: string | null) {
    const estado = this.normalizarEstado(nombre);
    return estado === 'aprobada' || estado === 'aprobado';
  }

  private esEstadoAnulada(nombre?: string | null) {
    const estado = this.normalizarEstado(nombre);
  ...
  async update(id: number, dto: UpdateOrdenVentaDto) {
    return this.prisma.$transaction(async (tx) => {
      ...
      if (!this.esEstadoPendiente(ordenActual.estado_orden_venta?.nombre_estado)) {
        throw new BadRequestException(
          'Solo se pueden editar órdenes de venta pendientes',
        );
      }

      const idClienteFinal = dto.id_cliente ?? ordenActual.id_cliente;
      const idBodegaFinal = dto.id_bodega ?? ordenActual.id_bodega;
      const idEstadoFinal = ordenActual.id_estado_orden_venta;
  ...
  async updateEstado(
    ...
    if (!this.esEstadoPendiente(orden.estado_orden_venta?.nombre_estado)) {
      throw new BadRequestException(
        'Solo se puede cambiar el estado de órdenes pendientes',
      );
    }

    await this.assertUsuarioExists(this.prisma, idUsuarioGestion);

    const nuevoEstado = await this.prisma.estado_orden_venta.findUnique({
      where: {
        id_estado_orden_venta: dto.id_estado_orden_venta,
      },
      select: {
        id_estado_orden_venta: true,
        nombre_estado: true,
      },
    });
    ...
    const data: Prisma.orden_ventaUncheckedUpdateInput = {
      id_estado_orden_venta: dto.id_estado_orden_venta,
    };

    if (this.esEstadoAprobada(nuevoEstado.nombre_estado)) {
      data.fecha_aprobacion = new Date();
      data.id_usuario_aprobo = idUsuarioGestion;
    } else if (this.esEstadoAnulada(nuevoEstado.nombre_estado)) {
      data.fecha_anulacion = new Date();
      data.id_usuario_anulo = idUsuarioGestion;
    } else {
      throw new BadRequestException(
        'La orden de venta solo puede pasar a Aprobada o Anulada',
      );
    }
